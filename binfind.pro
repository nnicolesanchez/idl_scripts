function binfind,u,v
;Author: Alyson Brooks
;Date: April 4, 2007
;+
; ROUTINE:  binfind is my own modification to findex
;
; PURPOSE:  Compute "floating point index" into a table for use with
;           INTERPOLATE.
;	findex uses a binary search + interpolation to find the values
;	of interest.  If the exact value you are looking for in array v 
;	exists in array u, the position in the list of u is returned.  
;	However, if it doesn't exist, findex interpolates.  binfind has 
;	been modified so that -1 is returned if the value doesn't exist in u.
;
; USEAGE:   binfind, u, v
;
; INPUT:    
...
test = u[w]
interp = where(test ne v)
IF interp[0] NE -1 then w[interp] = -1
return, w
