pro find_shocked_rvir, phase, haloidoutput, WMAP1=wmap1
...
ind = where(lowr le max(0.1*rvir), nind, comp=ind2, ncomp=nind2)
for k=0L,nind-1 do accr[ind(k)] = min(where(smphase[ind(k)].radius lt max(0.1*rvir) and smphase[ind(k)].radius ge 0.))
FOR j=0L,nind2-1 do accr[ind2(j)] = min(where(smphase[ind2(j)].radius eq lowr[ind2(j)]))  
change = where(rvir lt max(0.1*rvir))
first = max(change)+1
ind = where(accr le first,nind)
for k=0L,nind-1 do accr[ind(k)] = min(where(smphase[ind(k)].grp eq halo))

mwrfits, accr, 'tracedtostep.fits', /create
...
shinfo[3,*] = saccr
noshock:
mwrfits, iords, 'shocked.iord.fits', /create
mwrfits, shinfo, 'shocked.info.fits', /create
