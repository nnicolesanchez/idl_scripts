pro find_shocked, phase, haloidoutput, WMAP1=wmap1
...
ind = where(lowr le max(30.0*a), nind, comp=ind2, ncomp=nind2)
for k=0L,nind-1 do accr[ind(k)] = min(where(smphase[ind(k)].radius lt max(30.*a) and smphase[ind(k)].radius ge 0.))
FOR j=0L,nind2-1 do accr[ind2(j)] = min(where(smphase[ind2(j)].radius eq lowr[ind2(j)]))  
change = where(rvir lt max(30.0*a))
first = max(change)+1
