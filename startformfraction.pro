pro startformfraction,plotter=plotter,plottitle=plottitle,dolegend=dolegend
...
;if not keyword_set(plottitle) then plottitle='differential stellar makeup'
...
istherefile = file_search('startformfraction.dat')
...
; initialize time
   ...
   for i=0,nfiles-1 do begin
    ...
    ; find stars in tform range
      ingal = where(grp eq haloid[i] AND tform gt t0 AND tform le time[i]/tunit,ningal)
...
;stop
   ...
   openw,lun,'startformfraction.dat',/get_lun,width=500
   ...
   openw,lun,'BHtformfraction.dat',/get_lun,width=500
   ...
   readcol,'BHtformfraction.dat',time,coldbhmass,clumpybhmass,shockedbhmass,earlybhmass,format='d',/silent
endelse
...
if keyword_set(plotter) then paperplot,filename='startformgasfraction.ps'

plot,redshifts,clumpystarmass/totalstarmass,xtit='Redshift',ytit='Differential Stellar Mass Fraction',xra=[10,3],xstyle=1,yra=[0,1],title=plottitle
  oplot,redshifts,clumpystarmass/totalstarmass,color=125,thick=4
  oplot,redshifts,coldstarmass/totalstarmass,color=80,thick=4
  oplot,redshifts,shockedstarmass/totalstarmass,color=240,thick=4
  oplot,redshifts,earlystarmass/totalstarmass,color=30,thick=4

if keyword_set(dolegend) then legend,['clumpy','cold','shocked','early'],colors=[125,80,240,30],lines=[0,0,0,0],/right,charsize=1
...
totalBHfraction = clumpyBHmass+coldBHmass+earlyBHmass+shockedBHmass
    ...
    oplot,redshifts,earlyfraction,color=30,linestyle=2
