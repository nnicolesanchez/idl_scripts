pro massgastime_quies,plotter=plotter

restore,'../bhorbits.sav'
tunit=1.223128407d18/3.1556926d7/1d9 ; Gyr
munit=1.84793d16
restore,'allgas.sav'
readcol,'out.distance',w,bhiords,w,eattime,w,w,w,w,w,gasiords,dsquare,w,w,smooth,format='a,l,a,f,a,a,a,a,a,l,f,a,a,f,a,f',/silent
readcol,'out.dmq_eatenmass',w,bhiords,w,w,eatenmass,w,w,format='a,l,a,a,f,a,a',/silent
...
zplotnames=['12','10','8','7','6','5','4']
zplotnum=[12,10,8,7,6,5,4]
...
if keyword_set(plotter) then paperplot,filename='hz3_allmassgas.ps'

;### cumulative mass plot ###

; use orbit file to get max BH mass
bhorbitmass = fltarr(n_elements(bhorbits[1].x))
for i=0,n_elements(bhorbits[0].x)-1 do bhorbitmass[i] = max(bhorbits.mass[i])*munit
bhorbittime = bhorbits[0].time*tunit
;bhorbittime=bhorbittime[0:36]
;bhorbitmass=bhorbitmass[0:36]

bhorbittime=bhorbittime[where( bhorbitmass ne 0)]
...
plot,dmtime*tunit,alltotal,xtit='Time (Gyr)',ytit='Cumulative BH Mass',/ylog,yra=[100,1e7],xstyle=8
   oplot,clumpybhtimes*tunit,clumpytotal,color=140;,psym=3
   oplot,coldbhtimes*tunit,coldtotal,color=90;,psym=3
    if numshocked gt 0 then  oplot,shockedbhtimes*tunit,shockedtotal,color=240;,psym=3
;   oplot,earlybhtimes*tunit,earlytotal,color=50;,psym=3

  oplot,bhorbittime,bhorbitmass,linestyle=2,color=40

;legend,['total BH mass','total gas','cold','shocked','clumpy'],lines=[2,0,0,0,0],color=[40,0,90,240,140],charsize=1,/right,/bottom

axis,xaxis=1,xtit='Redshift',xtickv=tickvalues,xticks=n_elements(zplotnames)-1,xtickname=zplotnames

xyouts,0.1,4e6,'hz3',charsize=1.5,/data
...
plot,dmtime*tunit,alltotal,xtit='Time (Gyr)',ytit='Cumulative BH Mass (M'+sunsymbol()+')',/ylog,yra=[1,1e7];,title='hz3';,psym=3
