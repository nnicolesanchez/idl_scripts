pro metalgasfraction,plotter=plotter,plottitle=plottitle,dolegend=dolegend

;%%%%% UNFINISHED   DO NOT COMPILE  WILL NOT WORK %%%%%%%%
...
; a table.
metalfractiontable = file_search('metalfraction.dat')
if metalfractiontable[0] eq '' then begin
...
; eliminate early to get accrz for gas I care about
   ...
   for i=0,nfiles-1 do begin
    ...
    ; ID with iords and get masses
      ...
      coldmetals[i] = mean(g[ic2].zmetal)
      shockedmetals[i] = mean(g[is2].zmetal)
      clumpymetals[i] = mean(g[icl2].zmetal)
      earlymetals[i] = mean(g[e1].zmetal)
;    stat = mrdfits(file[i]+'.amigastat.fits',1)
;stop
endfor

openw,lun,'metalfraction.dat',/get_lun,width=500
printf,lun,'cold metals  ','clumpy metals  ','shocked metals   ','early metals   '
for i=0,nfiles-1 do printf,lun,coldmetals[i],clumpymetals[i],shockedmetals[i],earlymetals[i]
close,lun
free_lun,lun

endif else readcol,'metalfraction.dat',coldmetals,clumpymetals,shockedmetals,earlymetals,format='d',/silent


totalmetals = coldmetals+shockedmetals+clumpymetals+earlymetals

; normalize to total gas metals in halo 1?
; good sanity check to see if it adds up 

if keyword_set(plotter) then paperplot,filename='metalfraction.ps'

plot,redshifts,clumpymetals/totalmetals,xtit='Redshift',ytit='Gas Metals Fraction',xra=[11,3],xstyle=1,yra=[0,1],linestyle=0,title=plottitle
  oplot,redshifts,clumpymetals/totalmetals,color=125,linestyle=0;,psym=-1
  oplot,redshifts,coldmetals/totalmetals,color=80,linestyle=0
  oplot,redshifts,shockedmetals/totalmetals,color=240,linestyle=0
  oplot,redshifts,earlymetals/totalmetals,color=grey,linestyle=0
...
if n_elements(bhfile) ne 0 then begin
    ...
    redshift = 1./a-1.
    dmtimez = interpol(redshift,mdottime,bhaccretemetals.dmtime)
    ; OK now match times to each gas phase
    ; interpolate all times.
    clumpytotal = interpol(bhaccretemetals.clumpytotal,bhaccretemetals.clumpybhtimes,bhaccretemetals.dmtime)
    coldtotal = interpol(bhaccretemetals.coldtotal,bhaccretemetals.coldbhtimes,bhaccretemetals.dmtime)
    shockedtotal = interpol(bhaccretemetals.shockedtotal,bhaccretemetals.shockedbhtimes,bhaccretemetals.dmtime)
    earlytotal = interpol(bhaccretemetals.earlytotal,bhaccretemetals.earlybhtimes,bhaccretemetals.dmtime)
    ; interpolation is sucky for early at the end.  fix it.
    earlytotal[where(earlytotal ge max(bhaccretemetals.earlytotal))] =  max(bhaccretemetals.earlytotal)
    clumpyfraction = clumpytotal/bhaccretemetals.alltotal
    coldfraction = coldtotal/bhaccretemetals.alltotal
    shockedfraction = shockedtotal/bhaccretemetals.alltotal
    earlyfraction = earlytotal/bhaccretemetals.alltotal
...
; testing
;plot,bhaccretemetals.dmtime,bhaccretemetals.alltotal,/ylog,xra=[.01,.015]
;oplot,bhaccretemetals.shockedbhtimes,bhaccretemetals.shockedtotal,color=240
;oplot,bhaccretemetals.coldbhtimes,bhaccretemetals.coldtotal,color=80
;oplot,bhaccretemetals.clumpybhtimes,bhaccretemetals.clumpytotal,color=80
